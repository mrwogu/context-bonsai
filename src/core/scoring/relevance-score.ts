...
const RUST_BACKTRACE_FRAME_PATTERN =
  /^\s*\d+:\s+(?:0x[0-9a-f]+\s+-\s+)?\S*(?:::|[<>])\S*$/u;
const RUST_FILE_FRAME_PATTERN = /^\s*at\s+\S+\.rs:\d+:\d+:?$/u;
const CSHARP_STACK_FRAME_PATTERN =
  /^\s*at\s+[\w.<>`+\[\],$]+\([^)]*\)(?:\s+in\s+\S+:line\s+\d+)?$/u;
const PHP_STACK_FRAME_PATTERN =
  /^\s*#\d+\s+(?:\{main\}|\S+\(\d+\):\s+\S+.*)$/u;
...
const INTERNAL_STACK_PATTERN =
  /(?:node_modules[\\/]|node:internal|internal[\\/]modules|bootstrap_node|[\\/]usr[\\/]lib[\\/]|[\\/]usr[\\/]local[\\/]lib[\\/]|[\\/]usr[\\/]local[\\/]go[\\/]src[\\/]runtime[\\/]|site-packages[\\/]|dist-packages[\\/]|\.venv[\\/]|java\.base[\\/]|jdk\.internal|org\.springframework\.|[\\/]pkg[\\/]mod[\\/]|\.cargo[\\/]registry[\\/]|[\\/]gems[\\/]|vendor[\\/]bundle[\\/]|[\\/]rustc[\\/]|\bat System\.)/iu;
...
export function scoreLineRelevance(
  ...
  if (
    score > 0 &&
    seenCount === 1 &&
    inputLines >= RARITY_MIN_INPUT_LINES
  ) {
    score += RARITY_BOOST;
  }
