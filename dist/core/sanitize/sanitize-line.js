...
const K8S_RELATIVE_DURATION_PATTERN = /\b(?:\d+d)?(?:\d+h)?(?:\d+m)?\d+s\b/gu;
...
const IPV6_PATTERN = /(?:(?:[0-9a-fA-F]{1,4}:){2,}:[0-9a-fA-F]{1,4}(?::[0-9a-fA-F]{1,4})*|(?:[0-9a-fA-F]{1,4}:){4,}[0-9a-fA-F]{1,4}|::[0-9a-fA-F]{1,4}(?::[0-9a-fA-F]{1,4})+)/gu;
// ---- PII/demographic patterns ----
const EMAIL_PATTERN = /\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,24}\b/gu;
...
function sanitizeLine(line, preserveIdSuffix = 0) {
    const uuidReplacement = preserveIdSuffix > 0
        ? (m) => `[ID:${m.slice(-preserveIdSuffix)}]`
        : () => '[ID]';
    const hexHashReplacement = preserveIdSuffix > 0
